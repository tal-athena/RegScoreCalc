...
def exclude_stopwords(path, include=False):
    """
    ...
    text = nltk.Text(english)
    tagged = nltk.pos_tag(text)
    for term, pos in tagged:
        process_term(c, term, pos, not include)
    c.close()
    conn.commit()
    conn.close()
...
def is_alpha_str(s):
    return reduce(lambda x, y: x or y, map(lambda c: c.isalpha(), s))

def is_ngram_longer_than(s, N):
    strings = s.split()
    if len(strings) <= 1:
        return True # not ngram
    return reduce(lambda x, y: x and y, map(lambda c: len(c) > N, strings))

def is_complete_alpha_str_ngrams(s):
    strings = s.split()
    return reduce(lambda x, y: x and y, map(lambda c: is_complete_alpha_str(c), strings))

def is_complete_alpha_str(s):
    return reduce(lambda x, y: x and y, map(lambda c: c.isalpha(), s))
