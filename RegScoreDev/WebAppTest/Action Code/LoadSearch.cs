...
{
    class LoadSearch : Action
    {
        public LoadSearch()
            : base(new ActionControls.LoadSearcControl(), "Load Search")
        {
        ...
        {
            XmlNode node = document.CreateElement("LoadSearch");
        ...
        public override Result Run2(IWebDriver browser, Stopwatch sw, string URL)
        {
...

            {
                ...
                for (int i = 0; i <= refresh; i++)
                {
                    ...
                    browser.Navigate().GoToUrl(URL + "Search/SearchMain/Index/" + id);
                    browser.WaitForPageLoad();

                    if (browser.IsTextPresent("Server Error"))
                    {
                        throw new TestErrorException("Error while opening search with id: " + id, Result.Continue);
                    }
